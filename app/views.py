...
def namespaces(request):
    if request.GET.has_key('change_n'):
...
def repository(request):
    if request.GET.has_key('action') and request.GET['action'] == 'list_tags':
        repository = request.GET['repository']
        list = registry_api.get_repository_tags(repository)
        data = {}
        for tag in list:
    ...
    elif request.GET.has_key('action') and request.GET['action'] == 'delete_tag':
        repository = request.GET['repository']
        tag = request.GET['tag']
...
@login_required()
def pods(request):
    if request.GET.has_key('action') and request.GET['action'] == 'list_pods':
